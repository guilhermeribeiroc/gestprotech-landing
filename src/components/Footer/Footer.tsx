import React from 'react';
import { ArrowUpRight, Mail, ShieldCheck, Cloud } from 'lucide-react';
import styles from './Footer.module.css';

const WhatsAppIcon = () => (
  <svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor">
    <path d="M12.031 2c-5.502 0-9.97 4.468-9.97 9.97 0 1.916.545 3.701 1.488 5.216l-1.549 5.666 5.822-1.527a9.92 9.92 0 004.21 1.002c5.502 0 9.97-4.468 9.97-9.97C22.002 6.468 17.533 2 12.031 2zm0 17.159c-1.637 0-3.2-.444-4.57-1.285l-.328-.196-3.398.892.909-3.324-.215-.342a8.21 8.21 0 01-1.258-4.394c0-4.526 3.684-8.211 8.21-8.211 4.526 0 8.211 3.685 8.211 8.211 0 4.526-3.685 8.211-8.211 8.211zm4.512-6.177c-.247-.124-1.463-.722-1.69-.804-.226-.083-.392-.124-.556.124-.165.247-.638.804-.783.97-.144.165-.288.185-.535.061-.247-.124-1.042-.383-1.986-1.225-.733-.655-1.229-1.464-1.373-1.711-.144-.247-.015-.38.109-.503.111-.11.247-.288.371-.432.124-.144.165-.247.247-.412.083-.165.042-.309-.021-.433-.062-.124-.556-1.34-.762-1.836-.2-.486-.421-.42-.577-.428-.149-.008-.32-.01-.49-.01a.94.94 0 00-.68.32c-.227.247-.866.845-.866 2.062 0 1.216.887 2.392.986 2.556.103.165 1.745 2.665 4.227 3.733.59.254 1.05.406 1.41.52.593.189 1.133.162 1.56.098.476-.072 1.463-.598 1.669-1.175.206-.578.206-1.073.144-1.175-.062-.103-.227-.165-.474-.29z"/>
  </svg>
);

const Footer: React.FC = () => {
  return (
    <footer id="contato" className={styles.footerSection}>
      <div className={styles.container}>
        <div className={styles.grid}>
          {/* Coluna 1: Brand */}
          <div className={styles.brandCol}>
            <div className={styles.logoRow}>
              <img src="/LOGOGEST.png" alt="GestProTech" className={styles.logoImg} />
              <span className={styles.brandName}>GestProTech</span>
            </div>
            <p className={styles.brandDesc}>
              Plataforma de gestão definitiva para escritórios de advocacia que querem crescer com inteligência.
            </p>
            <a 
              href="https://otimizai.net.br" 
              target="_blank" 
              rel="noopener noreferrer" 
              className={styles.creatorLink}
            >
              <span className={styles.creatorLabel}>Feito por</span>
              <span className={styles.creatorName}>OtimizaAÍ</span>
              <ArrowUpRight size={10} className={styles.arrowIcon} />
            </a>
          </div>

          {/* Coluna 2: Plataforma */}
          <div>
            <h4 className={styles.titleCol}>Plataforma</h4>
            <ul className={styles.linksList}>
              <li><a href="#features" className={styles.linkItem}>Funcionalidades</a></li>
              <li><a href="#demo" className={styles.linkItem}>Ver o Sistema</a></li>
              <li><a href="#preco" className={styles.linkItem}>Preço</a></li>
              <li>
                <a 
                  href="https://gestprotech.com.br/registrar/" 
                  className={styles.linkItem}
                >
                  Acessar GestProTech
                </a>
              </li>
            </ul>
          </div>

          {/* Coluna 3: Contatos */}
          <div>
            <h4 className={styles.titleCol}>Contato</h4>
            <div className={styles.contactsList}>
              <a 
                href="https://wa.me/5588993699357" 
                target="_blank" 
                rel="noopener noreferrer" 
                className={styles.contactCard}
                style={{ background: 'rgba(37, 211, 102, 0.08)', border: '1px solid rgba(37, 211, 102, 0.18)' }}
              >
                <div className={styles.iconBox} style={{ background: '#25d366' }}>
                  <WhatsAppIcon />
                </div>
                <div className={styles.contactInfo}>
                  <span className={styles.contactTitle}>WhatsApp</span>
                  <span className={styles.contactValue}>(88) 99369-9357</span>
                </div>
              </a>

              <a 
                href="mailto:contato@otimizai.net.br" 
                className={styles.contactCard}
                style={{ background: 'rgba(45, 212, 191, 0.06)', border: '1px solid rgba(45, 212, 191, 0.14)' }}
              >
                <div className={styles.iconBox} style={{ background: 'linear-gradient(135deg, var(--teal-500), var(--teal-400))' }}>
                  <Mail size={16} />
                </div>
                <div className={styles.contactInfo}>
                  <span className={styles.contactTitle}>E-mail</span>
                  <span className={styles.contactValue}>contato@otimizai.net.br</span>
                </div>
              </a>
            </div>
          </div>
        </div>

        {/* Divider */}
        <div className={styles.divider}></div>

        {/* Bottom Row */}
        <div className={styles.bottomRow}>
          <div className={styles.copyrightText}>
            © 2026 <a href="https://otimizai.net.br" target="_blank" rel="noopener noreferrer">OtimizaAÍ</a>. Todos os direitos reservados.
          </div>
          <div className={styles.badgesRow}>
            <span className={styles.badgeItem}>
              <ShieldCheck size={14} className={styles.badgeIcon} />
              <span>Dados protegidos</span>
            </span>
            <span className={styles.badgeItem}>
              <Cloud size={14} className={styles.badgeIcon} />
              <span>99,9% uptime</span>
            </span>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;
